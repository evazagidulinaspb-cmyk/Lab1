...
> Дано неравенство: $\frac{x + a}{b\cdot x}$ ≥ 0, где a и b - параметры (вводятся с клавиатуры). Решите его для x.
...
- b - параметр, вещественное число. Количество: 1. Диапазон: любое вещественное число.
...
Рассмотрим выражение $\frac{x + a}{b\cdot x}$ ≥ 0. Область определения: x, b ∈ (-∞, 0)∪(0, +∞).
...
В итоге задача сводится к исследованию знака трёх множителей b, x, (x+a). Критические точки: x = 0, x = -a. В них неравенство будет менять знак, следовательно, именно эти точки будут в ответе. Точка -a всегда будет вколотой, а точка 0 выколотой.
...
- Для чтения входных значений используется объект Scanner. 
...
1) Вычислить ноль числителя = -a (для удобства далее -a = c).
...
- Output:
    ```
    Нет решений, так как b = 0, знаменатель не может быть равен нулю
    ```
