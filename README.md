...
В итоге задача сводится к исследованию знака трёх множителей b, x, (x+a).

Итоговый компактный вид решения:
 • Если b > 0: требуется x(x+a)\ge 0 при x\ne 0. То есть
x\in(-\infty,\min(-a,0)]\cup[\max(-a,0),+\infty)\quad\text{(с исключением }x=0\text{)}.
 • Если b<0: требуется x(x+a)\le 0 при x\ne 0. То есть
x\in[\min(-a,0),\max(-a,0)]\quad\text{(с исключением }x=0\text{)}.
 • Если b=0: выражение не определено для всех x → решений нет.

В разобранном по знаку a виде:

Если b>0:
 • a>0 (-a<0): x\in(-\infty,-a]\cup(0,\infty).
 • a=0: x\in(-\infty,0)\cup(0,\infty) (все x\neq0).
 • a<0 (-a>0): x\in(-\infty,0)\cup[-a,\infty).

Если b<0:
 • a>0 (-a<0): x\in[-a,0).
 • a=0: решений нет (для x\ne0 дробь равна 1/b<0).
 • a<0 (-a>0): x\in(0,-a].
